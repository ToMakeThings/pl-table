...
<script type="text/babel">
  ...
  export default {
    name: 'pl-table',
    components: {draggable, ElButton, ElTable, ElPagination},
    props: {
      heightSwitch: { type: Boolean, default: true }, // 是否需要去计算表格高度，关闭就相当于自适应高度，超出表格高度不会出现滚动条，开启相当于会去计算外层高度，等到表格高度，超出高度表格出现滚动条 (注意为false时候，请不要设置useVirtual属性)
    ...
    created() {
      ...
      // 是否有条件开启合并列
      if (typeof this.spanMethod === 'function' && !this.useVirtual && this.datas.length <= 50) {
    ...
    methods: {
              ...
              this.$nextTick(() => {
                ...
                this.$refs.singleTable.$el.childNodes.forEach(item => {
                  ...
                  if (item.className && item.className.indexOf('el-table__body-wrapper') !== -1) {
                    if (parseInt(item.style.height, 10)) {
                      if (item.children[0]) {
                        // 表体高度如果大于，表体内容的高度，代表表格高度得减少
                        let tablebodyH = parseInt(item.style.height, 10) - item.children[0].offsetHeight;
                        if (tablebodyH > 0) {
                          // 获取表格宽度  判断是否出现滚动条
      ...
      dropActionFun() {
        let dom = null;
        // 触发条件不修改表格的头部拖动样式， 开启了掉落元素
        if (!this.headerDragStyle && this.dropAction && document.getElementsByClassName('el-table__header')[0]) {
          dom = document.getElementsByClassName('el-table__header')[0].getElementsByClassName('has-gutter')[0];
